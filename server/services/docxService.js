...
/**
 * Create a professional DoF-styled Word document
  ...
  if (subtitle) {
    documentChildren.push(
      new Paragraph({
        ...
        spacing: { after: 400 },
        children: [new TextRun({ text: subtitle, italics: true, color: DOF_COLORS.gray })]
      })
    );
  }
  ...
  for (const section of sections) {
    ...
    if (section.content) {
      for (const item of section.content) {
        const contentElements = buildContentElement(item, numberingConfigs);
      ...
      paragraphStyles: [
        {
          ...
          name: 'Title',
          basedOn: 'Normal',
          run: { size: 48, bold: true, color: DOF_COLORS.navy, font: 'Arial' },
        ...
        },
        {
          ...
          next: 'Normal',
          quickFormat: true,
          run: { size: 36, bold: true, color: DOF_COLORS.seaGreen, font: 'Arial' },
        ...
        },
        {
          ...
          next: 'Normal',
          quickFormat: true,
          run: { size: 28, bold: true, color: DOF_COLORS.black, font: 'Arial' },
        ...
        },
        {
          ...
          next: 'Normal',
          quickFormat: true,
          run: { size: 24, bold: true, color: DOF_COLORS.gray, font: 'Arial' },
      ...
      headers: headerText ? {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [new TextRun({ text: headerText, color: DOF_COLORS.gray, size: 20 })]
          })]
        })
      } : undefined,
      footers: {
        default: new Footer({
          children: [new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [
              new TextRun({ text: footerText || 'Department of Finance', color: DOF_COLORS.gray, size: 18 }),
              new TextRun({ text: '  |  Page ', color: DOF_COLORS.gray, size: 18 }),
              new TextRun({ children: [PageNumber.CURRENT], color: DOF_COLORS.gray, size: 18 }),
              new TextRun({ text: ' of ', color: DOF_COLORS.gray, size: 18 }),
              new TextRun({ children: [PageNumber.TOTAL_PAGES], color: DOF_COLORS.gray, size: 18 })
            ]
          })]
        })
      },
 ...
 */
function buildContentElement(item, numberingConfigs) {
  const elements = [];
    ...
    case 'highlight':
      elements.push(
        new Paragraph({
          ...
          shading: { fill: DOF_COLORS.lightTeal, type: ShadingType.CLEAR },
          indent: { left: 360, right: 360 },
          children: [
            item.title ? new TextRun({ text: item.title + ': ', bold: true, color: DOF_COLORS.navy }) : null,
      ...
      new TableRow({
        ...
        children: headers.map(header =>
          new TableCell({
            ...
            shading: { fill: DOF_COLORS.seaGreen, type: ShadingType.CLEAR },
            verticalAlign: VerticalAlign.CENTER,
            children: [new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: header, bold: true, color: DOF_COLORS.white })]
            })]
          })
        )
      })
